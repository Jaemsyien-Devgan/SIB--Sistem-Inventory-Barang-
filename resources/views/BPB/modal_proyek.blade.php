<div id="modal_proyek" class="fixed z-10 inset-0 overflow-y-auto hidden" aria-labelledby="modal-title"
                        ...
                        <div class="p-6">
                                            ...
                                            @foreach ($lpb as $item)
                                                <tr>
                                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-white">
                                                        {{ $item->administrasi->proyek->kode_proyek }}</td>
                                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-300">
                                                        {{ $item->administrasi->proyek->nama_proyek }}</td>
                                                    <td class="px-4 py-2 whitespace-nowrap text-sm font-medium">
                                                        <button
                                                            ...
                                                            onclick="selectProyek('{{ $item->id }}','{{ $item->administrasi->proyek->kode_proyek }}', '{{ $item->administrasi->proyek->nama_proyek }}')">
                            ...
                            <div class="mt-6 flex justify-end space-x-3">
                                <button class="bg-gray-600 hover:bg-gray-500 text-white font-bold py-2 px-4 rounded"
                                    onclick="closeModal('modal_proyek')">Close</button>
                            </div>
                        </div>
                    </div>
                </div>
