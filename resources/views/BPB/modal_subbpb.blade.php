<div id="subbpb" class="fixed z-10 inset-0 overflow-y-auto hidden" aria-labelledby="modal-title" role="dialog"
    aria-modal="true">
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div
            ...
            <div class="p-6">
                <h2 class="text-lg font-medium text-gray-100">Daftar Produk</h2>
                <p class="mt-1 text-sm text-gray-400">Pilih Daftar Produk...</p>
                <div class="mt-4">
                                    ...
                                    <tr>
                                        ...
                                        <td class="px-4 py-2 whitespace-nowrap text-sm font-medium">
                                            <button
                                                ...
                                                onclick="selectbpb( '{{ $item->product->kode_produk }}', '{{ $item->product->nama_produk }}', '{{ $item->product->satuan->nama_satuan }}','{{$item->kuantitas}}','{{ $item->id }}')">
                ...
                <div class="mt-6 flex justify-end space-x-3">
                    <button class="bg-gray-600 hover:bg-gray-500 text-white font-bold py-2 px-4 rounded"
                        onclick="closeModal('subbpb')">Close</button>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    function selectbpb(kode_produk, nama_produk, nama_satuan, kuantitas,sub_lpb_id) {
        ...
        document.getElementById('sub_lpb_id').value = sub_lpb_id;

        // Menambahkan product_id ke tabel
        addProductToTable( kode_produk, nama_produk, nama_satuan, kuantitas);

        // Menutup modal setelah pemilihan produk selesai
        closeModal('subbpb');
    }

    function addProductToTable(kode_produk, nama_produk, nama_satuan, kuantitas) {
        ...
        row.innerHTML = `
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-300">${kuantitas}</td>
            <td class="px-4 py-2 whitespace-nowrap text-sm font-medium">
