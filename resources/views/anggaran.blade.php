...
@section('content')
            ...
            <div class="bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg border border-gray-700 border-1 p-4">
                ...
                <form action="{{ route('anggaran.store') }}" method="POST">
                    ...
                    <div class="p-6 text-gray-100 grid gap-6">
                        <div class="grid grid-cols-2 gap-6">
                            <div>
                                <label for="kode_anggaran" class="block text-sm font-medium">Kode Anggaran</label>
                                <div class="relative">
                                    <input id="kode_anggaran"
                                        class="block mt-1 w-full pl-10 bg-gray-700 border-gray-600 rounded-md"
                                        type="text" name="kode_anggaran" value="{{ $nextKodeAnggaran }}" readonly
                                        autofocus />
                            ...
                            <div>
                                <label for="nama_anggaran" class="block text-sm font-medium">Nama Anggaran</label>
                                <div class="relative">
                                    <input id="nama_anggaran"
                                        class="block mt-1 w-full pl-10 bg-gray-700 border-gray-600 rounded-md"
                                        type="text" name="nama_anggaran" value="{{ old('nama_anggaran') }}" required />
                                    <div class="absolute inset-y-0 left-0 flex items-center pl-3">
    ...
    <div class="p-6 bg-gray-100 dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
        ...
        <div>
            <div class="mb-4 flex flex-col sm:flex-row justify-between items-center">
                ...
                <form id="searchForm" method="GET" action="{{ route('anggaran.index') }}">
                    <div class="relative">
            ...
            <div class="overflow-x-auto bg-white dark:bg-gray-800 rounded-lg shadow overflow-y-auto">
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="px-6 py-4 text-sm text-gray-500 dark:text-gray-300 text-center">
                                    Tidak ada Anggaran yang ditemukan.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
                ...
                @if ($anggarans instanceof \Illuminate\Pagination\LengthAwarePaginator)
                    <div class="m-6">
                        {{ $anggarans->appends(request()->query())->links() }}
                    </div>
                @endif
            </div>
        </div>
    ...
    @foreach ($anggarans as $anggaran)
        <div id="modal_anggaran_{{ $anggaran->id }}" class="fixed z-10 inset-0 overflow-y-auto hidden"
                ...
                <div
                    ...
                    <div class="p-6">
                        ...
                        <div
                            ...
                            <!-- Edit Form -->
                            <form id="editForm" method="POST" action="{{ route('anggaran.update', $anggaran->id) }}">
                                @csrf
                                @method('PUT')
                                <div class="p-6 text-gray-100 grid gap-6">
                                    <div class="grid grid-cols-2 gap-6">
                                        <div>
                                            ...
                                            <div class="relative">
                                                <input id="kode_anggaran"
                                                    class="block mt-1 w-full pl-10 bg-gray-700 border-gray-600 rounded-md"
                                                    type="text" name="kode_anggaran" value="{{ $anggaran->kode_anggaran }}"
                                                    readonly autofocus />
                                                <div class="absolute inset-y-0 left-0 flex items-center pl-3">
                                                    <i class="fa-solid fa-clipboard-list h-5 w-5 text-yellow-400"></i>
                                                </div>
                                        ...
                                        <div>
                                            ...
                                            <div class="relative">
                                                <input id="nama_anggaran"
                                                    class="block mt-1 w-full pl-10 bg-gray-700 border-gray-600 rounded-md"
                                                    type="text" name="nama_anggaran" value="{{ $anggaran->nama_anggaran }}"
                                                    required />
                                                <div class="absolute inset-y-0 left-0 flex items-center pl-3">
                                                    <i class="fa-solid fa-file-signature h-5 w-5 text-red-400"></i>
                                                </div>
                                ...
                                <div class="mt-6 flex justify-end space-x-3">
                                    <button type="button"
                                        ...
                                        onclick="closeModal({{ $anggaran->id }})">
                                        Close
                                    </button>
                                    <button type="submit"
    ...
    <script>
        function openModal(id) {
            ...
            document.getElementById(`modal_anggaran_${id}`).classList.remove('hidden');
        }

        function closeModal(id) {
            // Menambahkan class 'hidden' untuk modal dengan ID yang sesuai
            document.getElementById(`modal_anggaran_${id}`).classList.add('hidden');
        }
